	...
	<view class="tui-container">
		<view class="tui-upload-box">
			<view class="tui-image-item" v-for="(item,index) in imageList" :key="index">
				<image :src="locallPath + '/uploads/images/' + item + '/view'" class="tui-item-img" @tap.stop="previewImage(index)" mode="aspectFill"></image>
		...
		methods: {
			...
			uploadImage: function(index, url) {
				...
				return new Promise((resolve, reject) => {
					uni.uploadFile({
						url: this.locallPath + this.serverUrl,
						...
						success: function(res) {
							...
							if (res.statusCode == 200) {
								...
								d._id && (_this.imageList[index] = d._id)
								_this.$set(_this.statusArr, index, d._id ? "1" : "3")
								resolve(index)
							} else {
			...
			previewImage: function(index) {
				...
				uni.previewImage({
					...
					urls: this.imageList.map(item => {
						return this.locallPath + '/uploads/images/' + item + '/view'
					})
				})
			}
		}
